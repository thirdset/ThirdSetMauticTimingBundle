...
/**
 * Older versions of Mautic don't have the ScheduledExecutioner class so we extend
 * a middleman class to maintain backwards compatibility.
 */
if (class_exists('\Mautic\CampaignBundle\Executioner\ScheduledExecutioner')) {
    class VersionSafeScheduledExecutioner extends \Mautic\CampaignBundle\Executioner\ScheduledExecutioner {}
} else {
    /**
     * Create a class to extend (we won't use it but this will prevent a
     * ClassNotFoundException in older versions of Mautic).
     */
    class VersionSafeScheduledExecutioner {}
}

/**
 * The ScheduledExecutioner class extends Mautic's ScheduledExecutioner class to add
 * our additional timing and scheduling logic. 
 * 
 * We extend via a middle man class called VersionSafeScheduledExecutioner 
 ...
 * @since 1.2
 */
class ScheduledExecutioner extends VersionSafeScheduledExecutioner
{   
    /**
    ...
    private function validateSchedule(Event $event, ArrayCollection $logs, \DateTime $now, $scheduleTogether = false)
    {
        ...
        foreach ($logs as $key => $log) {
            ...
            $this->scheduler->setCurrentContact($log->getLead());
            //////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
